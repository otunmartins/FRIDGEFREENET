...
function formatMessage(content) {
    ...
    // Handle special PSMILES workflow content
    let formatted = content;
    
    // Only skip markdown processing if content is ALREADY properly formatted HTML with buttons
    // Don't skip for mixed markdown/HTML content
    if (formatted.includes('data-action=') || formatted.includes('<button')) {
        // This is likely HTML content generated by the LLM that should be rendered as-is
        // Clean up any escaped quotes or malformed HTML
        formatted = formatted
            .replace(/&quot;/g, '"')
            .replace(/&lt;/g, '<')
            .replace(/&gt;/g, '>')
    ...
    // First, protect PSMILES [*] symbols from markdown processing
    const psmilesList = [];
    ...
    // Protect code blocks and inline code from markdown processing
    const codeBlocks = [];
    const inlineCodes = [];
    ...
    // Extract and temporarily replace inline code (including PSMILES)
