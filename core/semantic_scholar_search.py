...
def search_papers(client: SemanticScholar, query: str, limit: int = 10) -> List[Dict[str, Any]]:
    ...
    # Use islice to only take the first 'limit' results and avoid excessive pagination
    limited_results = itertools.islice(results, limit)
...
def get_paper_details(client: SemanticScholar, paper_id: str):
    """Get details of a specific paper."""
    return client.get_paper(paper_id)

def get_author_details(client: SemanticScholar, author_id: str):
    """Get details of a specific author."""
    return client.get_author(author_id)

def get_citations_and_references(paper) -> Dict[str, List[Dict[str, Any]]]:
