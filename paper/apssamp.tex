...
\title{Discovery of Insulin Delivery Polymers via Physics-Grounded Agentic Workflows}

\author[1]{Otun Martins}
\affiliation[1]{AI-Driven Material Discovery Laboratory}

\abstract{%
Over 500 million people with diabetes depend on insulin that loses potency outside 2--8$^{\circ}$C storage, yet the polymer space for thermally protective transdermal matrices is far too large for empirical screening.
We introduce an agentic discovery platform that couples a large language model (LLM) with physics-based molecular simulation through the Model Context Protocol (MCP), framing the search as budget-constrained black-box optimization over PSMILES polymer representations.
The LLM acts as an implicit acquisition function conditioned on a persistent ``discovery world'' state---a structured record of hypotheses, literature claims, and simulation outcomes that is updated after every iteration.
Under matched oracle budgets, the best agentic campaign achieves an insulin--polymer interaction energy of $-2263$~kJ/mol, outperforming reinforcement-learning baselines by 68\% and Bayesian optimization by 19\%.
...
Because the simulation stage is CPU-bound, the entire workflow runs on commodity hardware, aligning with emerging on-device and edge LLM deployments.
The platform generalizes to any protein-stabilization problem where a computationally tractable screening oracle exists.}
...
Yet none of these systems couples LLM-driven reasoning with explicit molecular dynamics in a loop where physics can falsify proposals before they shape the next hypothesis.

We address this gap with an agentic discovery platform built on the Model Context Protocol (MCP)~\cite{mcp2024}.
...
Improving $f$---through NPT dynamics with the UMA universal force field~\cite{wood2025uma} or free-energy perturbation---would sharpen the optimization landscape and likely amplify the agentic advantage.
...
Replacing the current single-point screening oracle with higher-fidelity molecular dynamics (explicit solvent, free-energy perturbation, universal force fields~\cite{wood2025uma}) would sharpen the optimization landscape and is the natural next step.
