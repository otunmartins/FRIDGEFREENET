#!/usr/bin/env python3
"""
OpenMM matrix encapsulation: insulin surrounded by polymer shell (Packmol),
...
def main() -> None:
    ...
    ap.add_argument("--box-nm", type=float, default=9.0)
    ap.add_argument("--shell-angstrom", type=float, default=14.0)
    ap.add_argument(
        "--density-driven",
        action="store_true",
        help="Use target density as primary; derive n_polymers and shell from geometry",
    )
    ap.add_argument(
        "--target-density",
        type=float,
    ...
    kw: dict = dict(
        ...
        restrain_shell=not args.no_restrain_shell,
