...
"""
Simplified MM-GBSA Calculator following proven OpenMM patterns
Based on OpenMM GitHub discussion: https://github.com/openmm/openmm/issues/3107

Uses the simple approach recommended by peastman:
- Load trajectory with MDTraj 
- Create systems once for complex, receptor, ligand
- Loop through frames updating positions only
- Calculate energies directly
"""
...
# OpenFF for small molecule handling
try:
    from openff.toolkit import Molecule
    from openmmforcefields.generators import SMIRNOFFTemplateGenerator
    OPENFF_AVAILABLE = True
except ImportError as e:
    logging.warning(f"OpenFF not available: {e}")
    OPENFF_AVAILABLE = False

class SimplifiedMMGBSACalculator:
    """
    Simplified MM-GBSA calculator following proven OpenMM patterns
    
    This follows the exact approach recommended in OpenMM GitHub discussions:
    1. Load trajectory with MDTraj
    2. Create systems once (complex, receptor, ligand)
    3. Loop through frames, updating positions only
    4. Calculate binding energy = E_complex - E_receptor - E_ligand
    """
    
    def __init__(self, output_dir: str = "simplified_mmgbsa"):
        """Initialize the simplified calculator"""
        ...
        if not OPENFF_AVAILABLE:
            raise ImportError("OpenFF is required for small molecule handling")
        ...
        # Standard amino acid residues for splitting
        ...
        print(f"🧮 Simplified MM-GBSA Calculator initialized")
    ...
    def calculate_binding_energy(self, frames_pdb_file: str, 
                                simulation_id: str,
                                output_callback: Optional[Callable] = None) -> Dict[str, Any]:
        """
        Calculate MM-GBSA binding energy using simplified approach
        
        Args:
            frames_pdb_file: Path to trajectory PDB file
            simulation_id: Unique simulation identifier
            output_callback: Optional progress callback
        ...
        log(f"🧮 Starting simplified MM-GBSA calculation for {simulation_id}")
        log("=" * 60)
        
        try:
            # Step 1: Load trajectory with MDTraj (much more efficient)
            log("📽️ Loading trajectory with MDTraj...")
            trajectory = md.load(frames_pdb_file)
            ...
            first_frame_file = str(self.output_dir / f"{simulation_id}_frame0.pdb")
            ...
            log(f"✅ Component extraction complete:")
            ...
            # Step 6: Create simulation contexts
            ...
            # Step 7: Process frames using proven OpenMM approach
            ...
            for frame_idx in range(n_frames):
                ...
                # Update contexts with new positions (efficient!)
            ...
            analysis_results = self._analyze_binding_results(frame_results, simulation_id)
            
            log(f"✅ MM-GBSA calculation completed!")
            ...
            log(f"❌ MM-GBSA calculation failed: {str(e)}")
            traceback.print_exc()
            return {'success': False, 'error': str(e)}
    
    def _extract_component(self, topology, positions, component: str):
        """Extract receptor or ligand component from complex"""
        ...
        # Add bonds between atoms in the component
    ...
    def _extract_component_positions(self, frame_positions, topology, component: str):
        """Extract positions for component from full frame"""
        component_positions = []
    ...
    def _create_polymer_molecule_from_topology(self, topology):
        """Create OpenFF molecule from topology"""
    ...
    def _analyze_binding_results(self, frame_results: List[Dict], simulation_id: str) -> Dict[str, Any]:
        """Analyze binding energy results"""
        ...
        results_file = self.output_dir / f"{simulation_id}_detailed_results.csv"
        ...
        results = {
            ...
            'method': 'Simplified MM-GBSA (OpenMM proven approach)',
            'timestamp': datetime.now().isoformat()
        }
        
        # Save summary
        summary_file = self.output_dir / f"{simulation_id}_summary.json"
        with open(summary_file, 'w') as f:
            json.dump(results, f, indent=2)
        
        return results

def test_simplified_calculator():
    """Test the simplified calculator"""
    try:
        calculator = SimplifiedMMGBSACalculator()
        print("✅ Simplified MM-GBSA calculator test passed")
        return True
    except Exception as e:
...
if __name__ == "__main__":
    success = test_simplified_calculator()
